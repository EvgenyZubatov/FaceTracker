...
public class MainActivity extends AppCompatActivity implements BLEListener{
    ...
    private int currentServoPosition = 90;
    ...
    private final String TARGET_BLE_DEVICE_NAME = "Lab4";
    ...
    private class FaceTracker extends Tracker<Face> {
        ...
        public void onUpdate(FaceDetector.Detections<Face> detectionResults, Face face) {
            ...
            /*
            byte leftOpen = (byte) ((face.getIsLeftEyeOpenProbability() > 0.6) ? 0xFF : 0x00);
            byte rightOpen = (byte) ((face.getIsRightEyeOpenProbability() > 0.6) ? 0xFF : 0x00);
            byte[] buf = new byte[] { (byte) 0x01, (byte) leftOpen, (byte) rightOpen, (byte) 0x00, (byte) 0x00}; // 5-byte initialization

            /**/

            float faceCenterX = face.getPosition().x + face.getWidth()/2.0f;
            byte sensorPos = (byte)((faceCenterX*180.0f)/CAMERA_PREVIEW_WIDTH);
            if (sensorPos != currentServoPosition) {
                currentServoPosition = sensorPos;
                byte[] buf = new byte[] { (byte) 0x02, (byte) currentServoPosition, (byte) 0x00, (byte) 0x00, (byte) 0x00}; // 5-byte initialization
                // Send the data!
    ...
    public void onBleConnected() {
        ...
        // setup servo
        byte[] buf = new byte[] { (byte) 0x02, (byte) currentServoPosition, (byte) 0x00, (byte) 0x00, (byte) 0x00};
        mBLEDevice.sendData(buf);
    }
    ...
    public void onBleDataReceived(byte[] data) {
        ...
        if (data[0] == 2) {
            ...
            TextView textViewBleStatus = (TextView)findViewById(R.id.textDistance);
            textViewBleStatus.setText(String.format("%d", cm));
        }
    }
